...
public class ScheduleGridDefinition extends CustomGridDefinition
{
	...
	private class NumericValueResolver
	{
		...
		public GxRange rangeFromValues(Object from, Object to)
		{
			...
			if (hasSelectedRow() && realSelectedValue == realStart)
	...
	{
		if (mMinX == null)
			mMinX = getXDataValue((mXAxis.elementAt(0).optStringProperty(mXValuesFieldName)));
	...
	{
		if (mMaxX == null)
			mMaxX = getXDataValue(mXAxis.elementAt(mXAxis.size() - 1).optStringProperty(mXValuesFieldName));
	...
	{
		if (mMinY == null)
			mMinY = getYDataValue(mYAxis.elementAt(0).optStringProperty(mYValuesFieldName));
	...
	{
		if (mMaxY == null)
			mMaxY = getYDataValue(mYAxis.elementAt(mYAxis.size() - 1).optStringProperty(mYValuesFieldName));
