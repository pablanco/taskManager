...
public class ImageViewTouchBase extends ImageView implements IDisposable {
	...
	{
		// ContentMode values "scaleToFill", "scaleAspectFillKeepingAspect", "originalSize", "scaleToFitKeepingAspect"

		// imageView.setScaleType(); must remain in matrix for zoom image
		if (scaleType != ImageScaleType.NO_SCALE)
		{
			setFitToScreen(true);
		}
		//mBaseMatrix.setRectToRect(new RectF(0,0,this.getMeasuredWidth(),this.getMeasuredHeight()),new RectF(0,0,((View)this.getParent()).getMeasuredWidth(),((View)this.getParent()).getMeasuredHeight()),Matrix.ScaleToFit.CENTER);
	}
}
