...
import com.artech.base.metadata.theme.ThemeFormClassDefinition.TargetSize;
...
public class CallOptions implements Serializable
{
	...
	public static final String OPTION_TYPE = "Type"; //$NON-NLS-1$
	public static final String OPTION_ENTER_EFFECT = "EnterEffect"; //$NON-NLS-1$
	public static final String OPTION_EXIT_EFFECT = "ExitEffect"; //$NON-NLS-1$
	public static final String OPTION_TARGET = "Target"; //$NON-NLS-1$
	public static final String OPTION_TARGET_SIZE = "TargetSize"; //$NON-NLS-1$
	public static final String OPTION_TARGET_HEIGHT = "TargetHeight"; //$NON-NLS-1$
	public static final String OPTION_TARGET_WIDTH = "TargetWidth"; //$NON-NLS-1$
	...
	public void setFromClass(ThemeFormClassDefinition formClass)
	{
		if (formClass != null)
		{
			mCallType = CallType.tryParse(formClass.getCallType());
			mEnterEffect = Transitions.get(formClass.getEnterEffect());
			mExitEffect = Transitions.get(formClass.getExitEffect());
			mTargetName = formClass.getTargetName();
			setTargetSize(formClass.getTargetSize());
		}
	}

	private void setTargetSize(ThemeFormClassDefinition.TargetSize size)
	{
		setTargetSize(size.Name, size.CustomWidth, size.CustomHeight);
	}
	...
	private void setTargetSize(String name, DimensionValue customWidth, DimensionValue customHeight)
	{
		...
		{
			mTargetWidth = DimensionValue.percent(95);
			mTargetHeight = DimensionValue.percent(95);
		}
	}
	...
	static DimensionValue getDefaultTargetWidth()
	{
		if (Services.Device.getScreenOrientation() == Orientation.PORTRAIT)
			return DimensionValue.percent(95);
		else
