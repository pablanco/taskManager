...
class EntityDatabase
{
	...
	static class QUERY
	{
		static final String TABLE = "_Queries"; //$NON-NLS-1$

		static final String COL_ID = "ROWID"; //$NON-NLS-1$
		static final String COL_BASE_URI = "QueryBaseUri"; //$NON-NLS-1$
		static final String COL_PARAMETERS = "QueryParameters"; //$NON-NLS-1$
		static final String COL_SERVER_TIMESTAMP = "QueryServerTimestamp"; //$NON-NLS-1$
		static final String COL_CLIENT_TIMESTAMP = "QueryClientTimestamp"; //$NON-NLS-1$
		static final String COL_IS_COMPLETE = "QueryIsComplete"; //$NON-NLS-1$
		static final String COL_ROW_COUNT = "QueryRowCount"; //$NON-NLS-1$
		static final String COL_HASH = "QueryHash"; //$NON-NLS-1$
	}

	static class QUERY_ENTITY
	{
		static final String TABLE = "_QueryEntities"; //$NON-NLS-1$

		static final String COL_QUERY_ID = "QueryId"; // FK to ROWID of Queries table. //$NON-NLS-1$
		static final String COL_ENTITY_ID = "EntityId"; // FK to ROWID of corresponding Entity table. //$NON-NLS-1$
		static final String COL_ORDER = "ROWID"; // ROWID of QueryEntity is insertion order. //$NON-NLS-1$

		static final String ENTITIES_OF_QUERY =
			String.format("SELECT %s FROM %s WHERE %s = ?", COL_ENTITY_ID, TABLE, COL_QUERY_ID); //$NON-NLS-1$
	}

	static class ENTITY
	{
		static final String COL_ENTITY_ID = "ROWID"; //$NON-NLS-1$
		static final String COL_EXTRA_KEY_HASH = "__ExtraKeyHash"; //$NON-NLS-1$
		static final String COL_DUMMY = "__Dummy"; //$NON-NLS-1$
		// static final String COL_ENTITY_KEY = "_EntityKey";
	}

	static class OPERATION
	{
		static final String TABLE = "_Operations"; //$NON-NLS-1$

		static final String COL_ID = "ROWID"; //$NON-NLS-1$

		static final String COL_URI = "OperationUri"; //$NON-NLS-1$
		static final String COL_TYPE = "OperationType"; //$NON-NLS-1$
		static final String COL_DATA = "OperationData"; //$NON-NLS-1$
		static final String COL_DATAKEY = "OperationDataKey"; //$NON-NLS-1$
		static final String COL_STATUS = "OperationStatus"; //$NON-NLS-1$

		static final String COL_OPERATION_TIMESTAMP = "OperationStartTimestamp"; //$NON-NLS-1$
		static final String COL_STATUS_TIMESTAMP = "OperationStatusTimestamp"; //$NON-NLS-1$
	}
	...
	private void selectAll(TableDefinition table, QueryData query, List<Entity> result)
	{
		...
		String join = String.format("%1$s LEFT JOIN %2$s ON (%1$s.%3$s = %2$s.%4$s)", //$NON-NLS-1$
				table.getSqlName(), QUERY_ENTITY.TABLE, ENTITY.COL_ENTITY_ID, QUERY_ENTITY.COL_ENTITY_ID);

		String[] columns = EntityDatabaseHelper.columnNames(table.getSqlName(), table.getColumns());
		String selection = String.format("%s.%s = ?", QUERY_ENTITY.TABLE, QUERY_ENTITY.COL_QUERY_ID);  //$NON-NLS-1$
		String[] selectionArgs = new String[] { Long.toString(queryId) };
		String orderBy = String.format("%s.%s", QUERY_ENTITY.TABLE, QUERY_ENTITY.COL_ORDER); //$NON-NLS-1$
	...
	private static Entity readEntity(TableDefinition table, Cursor cursor)
	{
		...
		Entity entity = new Entity(entityStructure);
	...
	public void insert(String tableName, QueryData query, Collection<Entity> entities)
	{
		TableDefinition table = mDefinition.getTable(tableName);
		if (table != null)
	...
	public void insert(String tableName, QueryData query, Entity entity)
	{
		List<Entity> entities = new ArrayList<Entity>();
		entities.add(entity);
		insert(tableName, query, entities);
	}
	...
	private void insert(TableDefinition table, QueryData query, Collection<Entity> entities)
	{
		...
		try
		{
			...
			qeValues.put(QUERY_ENTITY.COL_QUERY_ID, queryId);
			for (Long entityId : entityIds)
			{
				qeValues.put(QUERY_ENTITY.COL_ENTITY_ID, entityId);

				try
				{
					mDatabase.insertOrThrow(QUERY_ENTITY.TABLE, null, qeValues);
				}
				...
				{
					Services.Log.Error("Error inserting " + qeValues, ex); //$NON-NLS-1$
				}
			}
	...
	public void deleteAll()
	{
		...
		try
		{
			for (TableDefinition table : mDefinition.getTables())
				mDatabase.delete(table.getSqlName(), null, null);

			for (String mdTable : EntityDatabaseHelper.metadataTableNames())
				mDatabase.delete(mdTable, null, null);
	...
	private void deleteAll(TableDefinition table, QueryData query)
	{
		...
		try
		{
			...
			String entityWhereClause = String.format("%s IN (%s)", ENTITY.COL_ENTITY_ID, QUERY_ENTITY.ENTITIES_OF_QUERY); //$NON-NLS-1$
			entityWhereClause += String.format(" AND NOT EXISTS (SELECT * FROM %s QE WHERE QE.%s = %s.%s and QE.%s <> ?)", QUERY_ENTITY.TABLE, QUERY_ENTITY.COL_ENTITY_ID, table.getSqlName(), ENTITY.COL_ENTITY_ID, QUERY_ENTITY.COL_QUERY_ID); //$NON-NLS-1$
			...
			String eqWhereClause = String.format("%s = ?", QUERY_ENTITY.COL_QUERY_ID); //$NON-NLS-1$
			mDatabase.delete(QUERY_ENTITY.TABLE, eqWhereClause, whereQueryIdArgs);

			// Delete from the Query table.
			String qWhereClause = String.format("%s = ?", QUERY.COL_ID); //$NON-NLS-1$
	...
	public QueryData readQueryData(QueryData query)
	{
		...
		String queryWhere = QUERY.COL_BASE_URI + " = ? AND " + QUERY.COL_PARAMETERS + " = ?"; //$NON-NLS-1$ //$NON-NLS-2$
	...
	private Iterable<String> getFieldValues(TableDefinition table, List<String> fields, String filter, int operator)
	{
		...
		final String STR_FIELD = "_Str"; //$NON-NLS-1$

		String subSelect = Strings.EMPTY;
		for (String field : fields)
		{
			if (subSelect.length() != 0)
				subSelect += " union "; //$NON-NLS-1$

			subSelect += String.format("select %s as %s from %s", field, STR_FIELD, table.getSqlName()); //$NON-NLS-1$
		}

		if (Services.Strings.hasValue(filter))
		{
			if (operator == SearchDefinition.OPERATOR_BEGINS_WITH)
				filter = Strings.toLowerCase(filter) + "%"; //$NON-NLS-1$
			else
				filter = "%" + Strings.toLowerCase(filter) + "%"; //$NON-NLS-1$ //$NON-NLS-2$
		}
		else
			filter = "%"; //$NON-NLS-1$

		Cursor cursor = mDatabase.query(true, "(" + subSelect + ")", new String[] { STR_FIELD },  //$NON-NLS-1$ //$NON-NLS-2$
				STR_FIELD + " like ?", new String[] { filter }, null, null, STR_FIELD, null); //$NON-NLS-1$
		try
		{
			while (cursor.moveToNext())
	...
	private long writeQueryId(QueryData query)
	{
		...
		else
		{
			...
			String uWhereClause = String.format("%s = ?", QUERY.COL_ID); //$NON-NLS-1$
