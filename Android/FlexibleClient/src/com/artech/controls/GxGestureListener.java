...
import static com.artech.controls.GxTouchEvents.*;
...
public class GxGestureListener
{
	...
	public GxGestureListener(Context context, Coordinator coordinator)
	{
		...
		SWIPE_MAX_OFF_PATH_FACTOR = 0.2; // Can veer at most 20% off path.
	}

	public void addView(View control)
	{
		if (mCoordinator.hasAnyEventHandler(control, ALL_EVENTS))
		{
	...
	private class ViewTouchListener extends GestureDetector.SimpleOnGestureListener implements View.OnTouchListener
	{
		...
		private final String[] FEEDBACK_EVENTS = new String[] { TAP, LONG_TAP, DOUBLE_TAP };
		...
		private ViewTouchListener(View view)
		{
			...
			if (!mCoordinator.hasAnyEventHandler(view, ALL_EVENTS))
			...
			mShowFeedback = mCoordinator.hasAnyEventHandler(view, FEEDBACK_EVENTS);
		}
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY)
		{
			try
			{
				...
				boolean isHorizontal = Math.abs(deltaX) > SWIPE_MIN_DISTANCE && Math.abs(velocityX) > SWIPE_THRESHOLD_VELOCITY;
				boolean isVertical = Math.abs(deltaY) > SWIPE_MIN_DISTANCE && Math.abs(velocityY) > SWIPE_THRESHOLD_VELOCITY;
				...
				if (isHorizontal && isVertical)
				{
					...
					if (Math.abs(deltaY / deltaX) > SWIPE_MAX_OFF_PATH_FACTOR)
						isHorizontal = false;

					if (Math.abs(deltaX / deltaY) > SWIPE_MAX_OFF_PATH_FACTOR)
						isVertical = false;
				}
			...
			{
				Services.Log.warning("Exception in onFling", e);
			}
