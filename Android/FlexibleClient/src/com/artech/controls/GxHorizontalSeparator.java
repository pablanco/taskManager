...
public class GxHorizontalSeparator
{
	...
	public GxHorizontalSeparator(LayoutItemDefinition parentItem)
	{
		this(parentItem, parentItem.getThemeClass());
	}
	 ...
	 */
	public GxHorizontalSeparator(LayoutItemDefinition parentItem, ThemeClassDefinition parentItemClass)
	{
		...
		if (parentItemClass != null)
		{
			...
			isVisible = parentItemClass.getBooleanProperty("sd_row_horizontal_line_separator", defaultIsVisible); //$NON-NLS-1$
			separatorClassName = parentItemClass.optStringProperty("ThemeHorizontalLineClassReference"); //$NON-NLS-1$
		}

		// Default separator is in HorizontalLine class.
		if (!Services.Strings.hasValue(separatorClassName))
			separatorClassName = "HorizontalLine"; //$NON-NLS-1$

		mIsVisible = isVisible;
		ThemeClassDefinition separatorClass = PlatformHelper.getThemeClass(separatorClassName);
		...
		if (separatorClass != null)
		{
			...
			mDrawable = ThemeUtils.createBackgroundDrawableFromClass(separatorClass, backgroundOptions);

			// Read height: if specified use that one, otherwise use image's height if set, otherwise use 1.
			String strDipHeight = separatorClass.optStringProperty("height"); //$NON-NLS-1$
