...
public class DateTimePicker extends RelativeLayout implements View.OnClickListener, OnDateChangedListener, OnTimeChangedListener {
	...
	public DateTimePicker(Context context, AttributeSet attrs, int defStyle) {
		...
		if (CompatibilityHelper.isHoneycomb() && Services.Device.getScreenSmallestWidth() < 720)
	...
	public void updateDate(int year, int monthOfYear, int dayOfMonth) {
		try {
			...
			Services.Log.Error("Invalid Date", String.valueOf(year) + "-" + String.valueOf(monthOfYear) + "-" + String.valueOf(dayOfMonth), ex); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		}
	}
	...
	{
		// BUGFIX: As described in the issue https://code.google.com/p/android/issues/detail?id=78861
		// neither OnDateChangedListener nor OnTimeChangedListener are fired when using DateTimePicker.
		if (CompatibilityHelper.isApiLevel(Build.VERSION_CODES.LOLLIPOP))
		{
			// We force a "sync" here.
			if (mShowDate)
