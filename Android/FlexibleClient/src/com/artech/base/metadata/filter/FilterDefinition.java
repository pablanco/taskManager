...
public class FilterDefinition extends DataSourceMemberDefinition
{
	...
	{
		super(parent);
		mFilterAttributes = new ArrayList<FilterAttributeDefinition>();
	}
	...
	public FilterDefinition(IDataSourceDefinition parent, INodeObject jsonData)
	{
		...
		INodeObject jsonSearch = jsonData.optNode("search"); //$NON-NLS-1$
		if (jsonSearch != null)
		{
		...
		INodeObject jsonAdvanced = jsonData.optNode("advanced"); //$NON-NLS-1$
		if (jsonAdvanced != null)
		{
			INodeCollection jsonAttributes = jsonAdvanced.optCollection("filterAttribute"); //$NON-NLS-1$
