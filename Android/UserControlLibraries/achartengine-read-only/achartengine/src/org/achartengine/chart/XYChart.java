...
public abstract class XYChart extends AbstractChart
{
  ...
  public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
    ...
    maxScaleNumber++;
    if (maxScaleNumber < 0) {
      //set values to Draw Empty XYChart
      maxScaleNumber = 1;
      setValuesDrawEmpty();
    }
    ...
    if (mRenderer.isShowAxes()) {
      ...
      boolean bottomXAxis = false;
      boolean rightYAxis = false;
      for (int i = 0; i < maxScaleNumber && !rightYAxis; i++) {
        rightYAxis = mRenderer.getYAxisAlign(i) == Align.RIGHT;
        bottomXAxis = mRenderer.getXAxisAlign(i) == XAlign.BOTTOM;
      }
      if (bottomXAxis) {
  ...
  private void setValuesDrawEmpty() {
    Calendar cal = new GregorianCalendar();
    mRenderer.setXAxisMax(cal.getTime().getTime());
    cal.add(Calendar.DATE, - 6); 
    mRenderer.setXAxisMin(cal.getTime().getTime());
